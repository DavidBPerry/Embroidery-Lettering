...
void setup() {
  ...
  PEmbroiderStart();
  image_TEST = loadImage("Y.png");
  E.stroke(255, 0, 0);
  E.stroke(0, 0, 255);
  drawDoublMossyCirlce(E, width/3, height/4*3, 1, int(random(40,80)));
  filterReline(E);
...
void drawMossyCircle(PEmbroiderGraphics E, int x, int y, int diam2, float z, float zigOffset, float arLen,PVector centOff, int diamOffset) {
  ...
  E_Ref.image(image_TEST,width/2-image_TEST.width/2,height/2-image_TEST.height/2);
